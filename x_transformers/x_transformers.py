...
class BetterRelativePositionBias(nn.Module):
    def __init__(self, dim_head, max_distance=16):
        super().__init__()
        self.max_distance = max_distance
        self.distance_embedding = nn.Embedding(
            2 * self.max_distance + 1, dim_head)

    def forward(self, qk_dots, queries, keys):
        position_ids_l = torch.arange(
            queries.shape[2], dtype=torch.long, device=queries.device).view(-1, 1)
...
class AttentionLayers(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.rel_pos = BetterRelativePositionBias(
            dim // heads, max_distance=rel_pos_max_distance) if rel_pos_bias else None
